...
import autoupdater.DownloadLatestVersionFromRepo;
import com.compomics.pepshellstandalone.StartUp;
...
public class StartupManager {
    ...
    private static class pepshellVisitor implements FileVisitor<Path> {
        ...
        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
            if (file.getFileName().getName(-1).toString().matches("pepshell.*/.jar")) {
                try {
                    DownloadLatestVersionFromRepo.downloadLatestVersionFromRepo(file);
