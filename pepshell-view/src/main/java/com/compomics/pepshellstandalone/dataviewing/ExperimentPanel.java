...
import com.compomics.pepshellstandalone.drawmodes.Proteins.DomainProteinDrawMode;
...
class ExperimentPanel extends javax.swing.JPanel {
    ...
    private DrawProteinPeptidesInterface proteinDrawMode = new IntensityPeptideDrawMode();
    ...
    private void formMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseMoved
        ...
        if (pepshellProtein != null) {
            pepshellProtein.getPeptideGroups().stream().filter((peptideGroup) -> (evt.getX() >= (int) Math.ceil((double) horizontalOffset + peptideGroup.getStartingAlignmentPosition() * DrawModeUtilities.getScale()) && evt.getX() <= (int) Math.ceil((double) horizontalOffset + peptideGroup.getEndAlignmentPosition() * DrawModeUtilities.getScale()))).map((peptideGroup) -> {
                //dirty
        ...
        if (pepshellProtein != null) {
            ...
            if (pepshellProtein.getProteinSequence().isEmpty() && ProgramVariables.USEINTERNETSOURCES) {
                try {
                    ...
                    ProgramVariables.exceptionBus.post(new ExceptionEvent("could not retrieve protein sequence for: "+ pepshellProtein.getOriginalAccession(),true, Level.WARN));
                } catch (IOException ex) {
                    ProgramVariables.exceptionBus.post(new ExceptionEvent("could not connect to the internet",true, Level.WARN));
                }
            }
            if (!pepshellProtein.getProteinSequence().isEmpty()) {
                if (recalculate) {
                    ...
                    if (!pepshellProtein.getDomains().isEmpty()) {
                        try {
                            DrawModeUtilities.setProteinAlpha(0.18f);
                            domainBackgroundDrawMode.drawProteinAndPeptides(pepshellProtein, g, offsetPoint, width, this.getHeight());
                        } catch (UndrawableException ex) {
                            ProgramVariables.exceptionBus.post(new ExceptionEvent("Could not draw domains for protein "+ pepshellProtein.getVisibleAccession(),true,Level.ERROR));
                        }
