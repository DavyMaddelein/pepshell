...
import com.compomics.pepshell.controllers.objectcontrollers.ProteinUtilities;
...
public class JmolPanel extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        pdbViewPanel1 = new com.compomics.pepshellstandalone.dataviewing.PdbViewPanel();
        sequenceCoveragePanel = new com.compomics.pepshellstandalone.dataviewing.SequenceCoveragePanel();
    ...
    private void PDBFileComboBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_PDBFileComboBoxActionPerformed
        if (PDBFileComboBox.getSelectedItem() != null && presentedPepshellProtein != null) {
            try {
                ...
                //TODO fix this
                PDBInfoText.setText(new ExternalStructureDataSource<>().getPDBDataForPDBName(PDBFileComboBox.getSelectedItem().toString()));
                //temp
                try {
                    if (showPDBSequenceCheckBox.isSelected()) {
                        sequenceCoveragePanel.showProteinCoverage(ProteinUtilities.fromThreeLetterToOneLetterAminoAcids(PDBDAO.getSequenceFromPdbFile(PDBDAO.getPdbFileInMem(PDBFileComboBox.getSelectedItem().toString()))), presentedPepshellProtein);
                    } else {
                        sequenceCoveragePanel.showProteinCoverage(presentedPepshellProtein.getProteinSequence(), presentedPepshellProtein);
                    }
                } catch (Exception e) {
                    ProgramVariables.exceptionBus.post(new ExceptionEvent("",true, Level.ERROR));

                }
                sequenceCoveragePanel.setInteractionCoverage(new ExternalStructureDataSource<>().getInteractionPartnersForPDBName(PDBFileComboBox.getSelectedItem().toString()));
                for (PeptideGroup aGroup : presentedPepshellProtein.getPeptideGroups()) {
                    pdbViewPanel1.executeScript("select " + aGroup.getRepresentativePeptide().getBeginningProteinMatch() + "-" + aGroup.getRepresentativePeptide().getEndProteinMatch() + "; colour green");
                }
    ...
    private void sequenceCoveragePanelMouseDragged(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_sequenceCoveragePanelMouseDragged
        ...
        if (presentedPepshellProtein != null) {
            ...
            for (InteractionPartner aPartner : new ExternalStructureDataSource<>().getInteractionPartnersForRange(presentedPepshellProtein, start, stop)) {
    ...
    private com.compomics.pepshellstandalone.dataviewing.PdbViewPanel pdbViewPanel1;
    private com.compomics.pepshellstandalone.dataviewing.SequenceCoveragePanel sequenceCoveragePanel;
    ...
    public void preparePDBPanelForProtein(PepshellProtein pepshellProtein) throws ConversionException {
        try {
            ...
            PDBFileComboBox.setModel(new DefaultComboBoxModel<>(new String[]{"no PDB files found "}));
        }
    }

    public void setPDBProtein(PepshellProtein pepshellProtein) {
