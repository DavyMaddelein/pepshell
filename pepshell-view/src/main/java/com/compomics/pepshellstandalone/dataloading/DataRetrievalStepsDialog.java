...
import com.compomics.pepshellstandalone.componentmodels.DataRetrievalStepsDialogListCellRenderer;
    ...
    private void fillRetrievalStepsList() {
        ...
        ProgramProperties.getInstance().getProperties().entrySet().stream().forEach((aClassReference) -> {
            ...
            try {
                ...
                ProgramVariables.exceptionBus.post(new ExceptionEvent(ex.getMessage(),false, Level.ERROR));
                failedClasses.add((String)aClassReference.getValue());
            }
        });

        if(!failedClasses.isEmpty()){
            ProgramVariables.exceptionBus.post(new ExceptionEvent("could not load classes: "+failedClasses.stream().collect(Collectors.joining("\n")),true,Level.ERROR));
        }
