        ...
        if (aPepshellProtein != null) {
            for (Experiment anExperiment : experimentGroup) {
                ...
                if (anExperiment.getProteins().indexOf(aPepshellProtein) != -1) {
                    ...
                    List<PeptideGroup> sortedCopy = protein.getPeptideGroups().stream().sorted(Comparator.comparing(e -> e.getRepresentativePeptide().getBeginningProteinMatch())).collect(Collectors.toList());
