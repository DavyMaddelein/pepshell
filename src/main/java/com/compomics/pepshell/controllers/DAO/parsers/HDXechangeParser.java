        ...
        while ((line = reader.readLine()) != null) {
            ...
            for (int i = 5; results.length + columncounter > i; i += columncounter) {
                ...
                if (!results[i + 4].isEmpty()) {
                    currentExperiment.getProteins().get(0).addPeptideGroup(new PeptideGroup().addPeptide(new QuantedPeptide(basePeptide.getSequence(), 0.0, Double.parseDouble(results[i + 4]))));
                }
            }
        }
