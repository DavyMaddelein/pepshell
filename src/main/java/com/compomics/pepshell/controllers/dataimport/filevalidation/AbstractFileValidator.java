 ...
 * <p/>
 * <p/>
 * It has three methods for validation, a validate file without metadata, which will always
 ...
 * <p/>
 * The third one is a convenience method where the file and the annotations are aggregated in a single object
 * <p/>
 ...
 */
public class AbstractFileValidator<T extends SeparatedvalueExperimentMetaData> implements FileValidatorInterface<T> {
     ...
     *
     * note that the validation happens on a 0 base array while the column number is expected to be given as 1 based input
     ...
     * @throws ValidationException if the validation failure needs more explanation
     */
    private boolean validateExperiment(String[] experimentColumns, SeparatedvalueExperimentMetaData metaData) throws ValidationException {
        boolean validated = true;
        if (experimentColumns[metaData.getProteinAccessionColumn() -1].length() == 0) validated = false;
        if (validated && experimentColumns[metaData.getPeptideSequence() -1].length() == 0) {validated = false;} else{}
    ...
    @Override
    public boolean canValidateFile(AnnotatedFile<T> anAnnotatedFile){
        return true;
    }
}
